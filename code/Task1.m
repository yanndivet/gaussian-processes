...
meanfunc = []; hyp.mean = [];
covfunc = @covSEiso; ell = -1; sf = 0; hyp.cov = [ell, sf];
...
title(sprintf('GP Regression with Optimized Hyperparameters\nLength Scale (l) = %.3f, Signal Variance (sf) = %.3f, Noise Variance (sn) = %.3f', exp(hyp_opt.cov(1)), exp(hyp_opt.cov(2)), exp(hyp_opt.lik)));